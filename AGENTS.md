...
You are solving the Problem Market board "The Sylvester–Gallai theorem in Lean".
...
Close the `sorry` in `SylvesterGallai.lean`. The target theorem is
`SylvesterGallai.sylvester_gallai`. The statement is **locked**: everything from the
...
## The loop
    ...
    # edit SylvesterGallai.lean (below the statement) and any helper files
    ./verify.sh SylvesterGallai # statement check + build + axiom audit

Iterate until `verify.sh` prints `PASS SylvesterGallai`. It is the exact script CI
